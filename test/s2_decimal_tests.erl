...
binary_test_() ->
    ...
    [
     {V, fun() -> R = s2_decimal:to_decimal(V, Opts) end}
    ...
    [
     {<<V/binary, " ", (list_to_binary(atom_to_list(R)))/binary>>,
      fun() ->
          ...
          D = s2_decimal:to_decimal(V, Opts),
          Res = s2_decimal:to_binary(D)
      end}
     || {V, P, R, Res} <- Tests
    ].
...
sum_test_() ->
    ...
    [
     {<<A/binary, $+, B/binary>>,
      fun() ->
          A1 = s2_decimal:to_decimal(A, Opts),
          B1 = s2_decimal:to_decimal(B, Opts),
          R = s2_decimal:to_binary(s2_decimal:add(A1, B1))
      end}
     || {A, B, R} <- Tests
    ].
    ...
    [
     {<<A/binary, $*, B/binary>>,
      fun() ->
          A1 = s2_decimal:to_decimal(A, Opts),
          B1 = s2_decimal:to_decimal(B, Opts),
          R = s2_decimal:to_binary(s2_decimal:mult(A1, B1))
      end}
     || {A, B, R} <- Tests
    ].
    ...
    [
     {<<A/binary, $/, B/binary>>,
      fun() ->
          A1 = s2_decimal:to_decimal(A, Opts),
          B1 = s2_decimal:to_decimal(B, Opts),
          R = s2_decimal:to_binary(s2_decimal:divide(A1, B1, Opts))
      end}
     || {A, B, R} <- Tests
    ].
...
from_float_test_() ->
    ...
    [
     {iolist_to_binary(io_lib:format("~p", [F])),
      fun() ->
          D = s2_decimal:to_decimal(F, Opts)
      end}
     || {D, F} <- Tests ++ NegTests
    ].
...
error_badarg_test_() ->
    ...
    [
     {D,
      fun() ->
          ?assertException(error, badarg, s2_decimal:to_decimal(D, Opts))
      end} || D <- Tests
    ].
...
cmp_test_() ->
    ...
    [
     {iolist_to_binary(
          [
           s2_decimal:to_binary(A),
           case R of
               1 -> $>;
               0 -> $=;
               -1 -> $<
           end,
           s2_decimal:to_binary(B)
          ]),
      fun() ->
          R = s2_decimal:cmp(A, B, Opts)
      end} || {A, B, R} <- Tests
    ].
...
to_binary_test_() ->
    ...
    [
     {iolist_to_binary(
      ...
      fun() ->
          R = s2_decimal:to_binary(D, #{pretty => P})
      end} || {P, D, R} <- Tests
    ].
